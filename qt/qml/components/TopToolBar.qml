...
MenuBar {
    ...
    // --- 1. Define all application actions ---
    // All logic, text, and shortcuts are defined here.
    // The Menus and ToolButtons below will just link to these.
    ...
    Menu {
        ...
        MenuItem { action: editDeleteAction }
        ToolSeparator {orientation: Qt.Horizontal}
        MenuItem { action: editFindAction }
        MenuItem { action: editPrefsAction }
    }

    Menu {
        title: "&View"

        MenuItem { action: viewZoomInAction }
        MenuItem { action: viewZoomOutAction }
        MenuItem { action: viewZoomResetAction }
        ToolSeparator {orientation: Qt.Horizontal}
        MenuItem { action: viewStatusBarAction }
        MenuItem { action: viewFullScreenAction }
    }

    Menu {
        title: "&Help"

        MenuItem { action: helpContentsAction }
        ToolSeparator {orientation: Qt.Horizontal}
